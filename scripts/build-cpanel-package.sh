...
APP_NAME="${APP_NAME:-hitung-barang-cpanel}"
APP_DIR_NAME="${APP_DIR_NAME:-hitung-barang}"
...
for command_name in git npm php composer rsync zip unzip sed grep awk sort paste; do
...
if [[ "$(git rev-parse --is-inside-work-tree 2>/dev/null || true)" != "true" ]]; then
    echo "ERROR: builder harus dijalankan dari checkout Git hitung-barang." >&2
    exit 1
fi

worktree_status="$(git status --porcelain --untracked-files=all)"
if [[ -n "$worktree_status" ]]; then
    echo "ERROR: working tree belum bersih. Commit, pindahkan, atau hapus perubahan berikut sebelum membuat paket:" >&2
    printf '%s\n' "$worktree_status" >&2
    exit 1
fi
...
if [[ -z "$(env_value APP_URL)" ]]; then
    echo "ERROR: APP_URL pada $ENV_FILE kosong." >&2
    exit 1
fi
...
if [[ -z "$SITE_URL" ]]; then
    SITE_URL="$(env_value APP_URL)"
fi
SITE_URL="${SITE_URL%/}"
packaged_url="$(env_value APP_URL)"
packaged_url="${packaged_url%/}"
if [[ "$SITE_URL" != "$packaged_url" ]]; then
    echo "ERROR: SITE_URL ($SITE_URL) harus sama dengan APP_URL pada $ENV_FILE ($packaged_url)." >&2
    exit 1
fi
...
{
    echo "hitung-barang cPanel deployment"
    echo "ZIP: $(basename "$zip_file")"
    echo "Extract the ZIP directly into the cPanel home directory."
    echo "Top-level folders in ZIP: $APP_DIR_NAME/ and $PUBLIC_DIR_NAME/."
    ...
    echo "$clear_file_name clears pre-bootstrap caches, refreshes web OPcache when enabled,"
    echo "verifies the production database, then runs optimize:clear, migrate --force, optimize, and deletes itself after success."
} > "$setup_file"
